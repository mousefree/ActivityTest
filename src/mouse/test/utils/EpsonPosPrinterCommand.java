 ...
 * http://blog.sina.com.cn/s/blog_53dd74430100kcym.html
 * http://iffiffj.iteye.com/blog/251351
 * http://www.thinksaas.cn/group/topic/257403/
 * http://wenku.baidu.com/link?url=TMEbpcDaCl36SUC4kuwHwqZ3WWoS_z1RhUQfIJ7p3SKyHQOU-271xZzJNoJMKXopTJlvDb2RU-9Xw5PFohJh8xw4raGRU1ozOQnsiriSJ-7
 * http://blog.csdn.net/whyisjava/article/details/8525443
 * http://blog.csdn.net/a1031359915/article/details/21465195  只有部分的打印指令
 * http://blog.sina.com.cn/s/blog_53dd74430100kcym.html
 */
public class EpsonPosPrinterCommand {
	...
	/*
	 * 进纸位置，当纸放上以后，调用此方法，可以直接让纸到达某一位置。
	 * 开发手册中的名称是实行单次n/180 跳行
	 */
	...
	/*
	 * 退纸位置，从当前位置直接让纸退到某一位置。
	 * 开发手册中的名称是实行单次n/180 逆向跳行，实行－8行。一次走8行？待测试
	 */
	public static byte[] printAndReturnPaper(int n) {
		n = (n > 255 ? 255 : n);
		n = (n < 0 ? 0 : n);
		return new byte[] { ESC, 'j', (byte) n };
	}
	 ...
	 * 换行
	 */
	public static byte[] setChangeLine() {
		return new byte[] {LF};
	}
	 ...
	 * nH 必须介于0 - 1 之间
	 */
	public static byte[] setLeft(int nL, int nH) {
		int n, n1, n2;
		// 这个是单位 英寸  根据打印机的不同，可能会需要不同的值，目前看到的资料有60，170
		int unit = 170;
		n1 = (nL > 255 ? 255 : nL);
		if(nH == 0 || nH == 1) {
			n2 = nH;
		...
		n = (n1 + n2 * 256) * (1 / unit);
		return new byte[] { ESC, '$', (byte) n1, (byte) n2 };
	}
